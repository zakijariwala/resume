...
Developer mode will eventually live at `/dev` — same pattern as Curious. Build the mode switcher to support this from the start: it receives a `currentMode` prop and routes accordingly.

The `/curious` page has:
- The full Award redesign (warm light mode: `~#f7f4ef` background, coral accent)
- A mode switcher in the nav (links back to `/?mode=recruiter` and `/?mode=developer`, links to `/dev` when built)
- A floating "← Back to main portfolio" button
- Astro View Transitions (`<ClientRouter />`) for smooth cross-page navigation

All three pages share `src/data/*.json` as the single source of truth. `meta.json` has three photo fields: `photo_recruiter`, `photo_developer`, `photo_curious`. Each page's Hero uses only its own.
